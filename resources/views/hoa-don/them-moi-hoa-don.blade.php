        ...
        <div class="col-lg-12">
            ...
            {!!Form::open(['action' => 'HoaDonController@store','method'=> 'POST']) !!}
                    ...
                    <div class="form-group">
                      ...
                      <div class="form-group">
                        <select class="form-control" id="dichvu" name="dich_vu" required focus>
                        <option value="" disabled selected>Chọn dịch vụ</option>        
                        @foreach($dichvu as $dich_vu)
                        <option name="dichvu" value="{{$dich_vu->id}}">{{ $dich_vu->ten_dich_vu }}</option>
                        @endforeach
                         </select>   
                     </div> 
                    </div>
                    <div class="form-group">
                      {{Form::label('title','Số lượng')}}<span class="text-danger"> *</span>
                      <div class="form-group">
                        {{Form::Text('so_luong','',['class'=> 'form-control','placeholder'=>'Nhập số lượng'])}}
                      </div> 
                    </div> 
                      <div class="form-group">
                       {{Form::label('title','Thành tiền')}}<span class="text-danger"> *</span>
                      <div class="form-group">
                        {{Form::Text('thanh_tien',"",['class'=> 'form-control', 'disabled'=>"disabled",'value'=>$thanhtien])}}
                      </div> 
                     </div> 
                    </div> 
                    </fieldset>
                    <div class="row">
                     <div class="form-group">
                     <button type ="button" class='btn btn-purple waves-effect waves-light'id ="btnAddService" name='add_service'>Thêm</button>
                      </div> 
                    </div>
                    <div class="form-group">
...
$(document).ready(function(){
  $("#btnAddService").click(function(){
    var rowTemplate = '<div class="row service"><div class="form-group"><select class="form-control" id="dichvu" name="dich_vu" required focus><option value="" disabled selected>Chọn dịch vụ</option>@foreach($dichvu as $dich_vu)<option name="dichvu" value="{{$dich_vu->id}}">{{ $dich_vu->ten_dich_vu }}</option>@endforeach</select></div><div class="form-group">{{Form::Text("so_luong",'',["class"=> "form-control","placeholder"=>"Nhập số lượng"])}}</div><div class="form-group">{{Form::Text("thanh_tien",'',["class"=> "form-control", "disabled"=>"disabled"])}}</div>  </div>'
